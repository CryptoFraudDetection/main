...
for _, row in tqdm(user_edges.iterrows()):
    G_weighted.add_edge(row["parent_user"], row["author"], weight=row["weight"])

plt.figure(figsize=(12, 10))
pos = nx.spring_layout(G_weighted, seed=42)  # Layout for visual clarity

# Draw nodes and edges
nx.draw_networkx_nodes(G_weighted, pos, node_size=30, alpha=0.7)
nx.draw_networkx_edges(G_weighted, pos, width=1.5, alpha=0.5, arrowstyle="->", arrowsize=10)

edge_labels = nx.get_edge_attributes(G_weighted, "weight")
for (u, v), label in edge_labels.items():
    x, y = (pos[u][0] + pos[v][0]) / 2, (pos[u][1] + pos[v][1]) / 2  # Midpoint of edge
    plt.text(x, y, f"{label:.1f}", fontsize=8, color="black", alpha=0.9)
...
```{python}
tqdm_desc = "Louvain Clustering"
with tqdm(total=1, desc=tqdm_desc) as pbar:
    partition = community_louvain.best_partition(G_weighted)
    pbar.update(1)

# Add the partition (cluster) to each node in the graph
for node, cluster in partition.items():
    G_weighted.nodes[node]['cluster'] = cluster

# Display the number of clusters
num_clusters = len(set(partition.values()))
print(f"Louvain clustering completed: {num_clusters} clusters found.")

# Visualize the clusters
plt.figure(figsize=(12, 10))
pos = nx.spring_layout(G_weighted, seed=42)  # Layout for visual clarity

# Draw nodes with color by cluster
cluster_colors = [partition[node] for node in G_weighted.nodes()]
nx.draw_networkx_nodes(G_weighted, pos, node_size=700, alpha=0.8, node_color=cluster_colors, cmap=plt.cm.Set3)
nx.draw_networkx_edges(G_weighted, pos, width=1.5, alpha=0.6, arrowstyle="->", arrowsize=15)
nx.draw_networkx_labels(G_weighted, pos, font_size=12, font_color="black")

plt.title("Louvain Clustering of User Interaction Graph", fontsize=16)
plt.axis("off")
plt.show()
```
